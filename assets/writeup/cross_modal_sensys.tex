...
DINOv2-TCN, subj.\ norm       & 0.717\,$\pm$\,0.037 & 0.636\,$\pm$\,0.011 & 0.262\,$\pm$\,0.050 \\
\midrule
\multicolumn{4}{l}{\textit{Cross-modal (IMU guidance, depth-only inference)}} \\
DINOv2-TCN + LIMU-BERT        & 0.697\,$\pm$\,0.047
                               & 0.645\,$\pm$\,0.033
                               & 0.247\,$\pm$\,0.058 \\
\bottomrule
\end{tabular}
...
The full 5-fold cross-modal model achieves $\text{AUC}=0.697 \pm 0.047$,
a $+0.059$ improvement over the population-normalised depth baseline ($0.638$)
but $-0.020$ below the subject-normalised baseline ($0.717$).
Balanced accuracy similarly improves from $0.587$ (pop.\ norm) to $0.645$,
and MCC from $0.173$ to $0.247$, while sensitivity ($0.654$) and specificity
($0.636$) are well-balanced, indicating the alignment objective successfully
suppresses class-specific bias.

These results indicate that cross-modal training with a frozen IMU teacher
transfers complementary stress-discriminative structure into the depth encoder,
yielding a $+9.2\%$ AUC gain over the unadapted depth baseline.
However, the cross-modal model does not yet close the gap to the
subject-normalised depth baseline, suggesting that the alignment loss alone
is insufficient to replicate the per-participant distributional correction
applied at subject-norm inference time.
A natural extension is to combine cross-modal training with subject
normalisation at inference, which we expect to recover and surpass the
$0.717$ ceiling.
