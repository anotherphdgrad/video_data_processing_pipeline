#!/usr/bin/env python3
"""
Cross-modal joint fine-tuning with MSE + KL + dual classification losses.

Variant of train_cross_modal.py where the IMU projection path is fully
trained (unfrozen) alongside the depth encoder.  The frozen teacher
embedding is projected through imu_in_proj and shared_mlp — neither of
which is frozen — so the shared space is supervised from both sides.

Loss
----
  L = L_cls_depth
    + λ_cls_imu  * L_cls_imu          -- IMU path classification
    + λ_mse      * L_mse              -- pre-shared projection alignment
    + λ_kl       * T² * L_kl          -- post-shared logit consistency

Where:
  h_depth     = depth_in_proj(encoder(x_depth))
  h_imu       = imu_in_proj(z_imu_frozen)     -- imu_embed NOT detached here
  z_depth     = shared_mlp(h_depth)
  z_imu       = shared_mlp(h_imu)
  logits_depth = classifier(z_depth)
  logits_imu   = classifier(z_imu)

  L_cls_depth = CE(logits_depth, labels)
  L_cls_imu   = CE(logits_imu,   labels)
  L_mse       = MSE(h_depth, h_imu)            -- both sides meet in the middle
  L_kl        = KL(softmax(logits_imu/T) || softmax(logits_depth/T))
                  depth is student, IMU path is teacher for KL direction

Note on "unfreezing": the LIMU-BERT encoder itself is never loaded during
training — its output (z_imu_frozen) comes from pre-extracted .npz files.
What IS unfrozen is the imu_in_proj and shared_mlp weights, which receive
gradients from L_cls_imu, L_mse, and L_kl in addition to L_cls_depth.
...
----------------------------------------------------
  ...
  Added   : lambda_cls_imu, lambda_mse, lambda_kl, temperature

Usage
-----
python train_cross_modal_joint.py --config config.json --run-name cross_modal_joint_v1

Smoke test:
python train_cross_modal_joint.py --config config.json \
    --run-name joint_smoke --max-folds 1 --optuna-trials 3
"""
...
def train_one_fold(
    *,
    ...
    lambda_cls_imu = float(params["lambda_cls_imu"])
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for depth_x, imu_embed, labels in train_loader:
            ...
            h_depth  = model.depth_in_proj(r_d)          # pre-shared repr
            z_depth  = model.shared_mlp(h_depth)
            logits_d = model.classifier(z_depth)
            loss_cls_depth = F.cross_entropy(logits_d, labels)

            # ── IMU path (imu_embed frozen, but imu_in_proj + shared_mlp trainable) ──
            # NOT detaching imu_embed: gradients flow through imu_in_proj and
            # shared_mlp from all three IMU-side losses below.
            h_imu    = model.imu_in_proj(imu_embed)      # pre-shared repr
            z_imu    = model.shared_mlp(h_imu)
            logits_i = model.classifier(z_imu)

            # L_cls_imu: IMU path learns to classify stress correctly
            loss_cls_imu = F.cross_entropy(logits_i, labels)

            # L_mse: pre-shared representations pulled together (both sides train)
            loss_mse = F.mse_loss(h_depth, h_imu)

            # L_kl: depth distribution matches IMU distribution in shared space
            # Detach IMU logits so KL only trains the depth path for this term
            loss_kl = F.kl_div(
...
def sample_params(trial: optuna.Trial) -> dict:
    return {
        ...
        "lambda_cls_imu": trial.suggest_float("lambda_cls_imu", 0.1,  1.0),
        "lambda_mse":     trial.suggest_float("lambda_mse",     0.01, 1.0, log=True),
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Cross-modal depth training: joint MSE + KL + dual classification."
    )
...
def main():
    ...
    run_name    = args.run_name or datetime.now().strftime("cross_modal_joint_%Y%m%d_%H%M%S")
    ...
    config = vars(args)
    config.update({"imu_dim": imu_dim, "n_pairs": dataset.n_pairs_total, "variant": "joint"})
